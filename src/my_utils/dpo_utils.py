...
def parse_args_paired_training(input_args=None):
   ...
   """
    ...
    parser.add_argument("--train_method", type=str, default="gan", choices=["gan", "dpo"])
...
def parse_args_dpo():
    ...
    Extract DPO options, parse the remaining options with the base parser, then merge them.
    """
    import sys
    
    # 1. Define DPO-specific arguments.
    parser = argparse.ArgumentParser(add_help=False) # Disable automatic help here to avoid conflicting -h options.
    parser.add_argument("--beta_dpo", type=float, default=0.1, help="DPO KL penalty strength")
    parser.add_argument("--ref_model_path", type=str, default=None, help="Path to reference model checkpoint")
    parser.add_argument("--dpo_latent_reward_weight", type=float, default=1.0,
                        help="Weight for latent-space chosen/rejected reward in DPO.")
    parser.add_argument("--dpo_image_reward_weight", type=float, default=0.25,
                        help="Weight for image-space chosen/rejected reward in DPO.")
    parser.add_argument("--dpo_anchor_weight", type=float, default=0.05,
                        help="Small supervised anchor on the chosen target to stabilize DPO.")
    
    # 2. Parse only the DPO-specific arguments.
    # Store DPO settings separately and pass remaining options, such as dataset_folder, onward.
    dpo_args, remaining_argv = parser.parse_known_args()
    
    # 3. Parse the remaining arguments with the base parser.
    # Passing only remaining_argv prevents the base parser from rejecting DPO-specific options.
    args = parse_args_paired_training(input_args=remaining_argv)
    
    # 4. Merge DPO options into the main arguments object.
    args.beta_dpo = dpo_args.beta_dpo
    args.ref_model_path = dpo_args.ref_model_path
    args.dpo_latent_reward_weight = dpo_args.dpo_latent_reward_weight
    args.dpo_image_reward_weight = dpo_args.dpo_image_reward_weight
    args.dpo_anchor_weight = dpo_args.dpo_anchor_weight
...
def parse_args_unpaired_training():
   ...
   """
    ...
    parser.add_argument("--dataloader_num_workers", type=int, default=0)
    parser.add_argument("--train_batch_size", type=int, default=4, help="Batch size (per device) for the training dataloader.")
    parser.add_argument("--train_encoder_lora", action="store_true", help="Whether to train/load LoRA for the VAE encoder.")
...
class DPODataset(torch.utils.data.Dataset):
    def __init__(self, dataset_folder, split, image_prep, tokenizer):
        """
        DPO Dataset for Pix2Pix Turbo.
        Expected folder structure under dataset_folder:
            /input  -> Source images (Conditioning)
            /good   -> Preferred images (Winner)
            /bad    -> Non-preferred images (Loser)
        
        It reuses the build_transform and normalization logic from PairedDataset.
        """
        super().__init__()
        self.split = split
        self.tokenizer = tokenizer
        
        # Set the triplet image directories.
        self.input_folder = os.path.join(dataset_folder, "input")
        self.good_folder = os.path.join(dataset_folder, "good")
        self.bad_folder = os.path.join(dataset_folder, "bad")
        
        # Load shared image captions from input_prompts.json.
        # Without a prompt JSON file, enumerate input filenames and use empty captions below.
        prompts_file = os.path.join(dataset_folder, "input_prompts.json")
        if os.path.exists(prompts_file):
            with open(prompts_file, "r") as f:
                self.captions = json.load(f)
            self.img_names = list(self.captions.keys())
        else:
            print(f"Warning: prompts.json not found in {dataset_folder}, using filenames as prompts.")
            self.captions = None
            self.img_names = sorted(os.listdir(self.input_folder))
            # Filter out non-image filenames.
            self.img_names = [x for x in self.img_names if x.endswith(('.png', '.jpg', '.jpeg', '.tif', '.tiff'))]

        # Reuse the existing image transform builder.
        self.T = build_transform(image_prep)
    ...
    def __getitem__(self, idx):
        ...
        # 1. Load the image triplet.
        # Retry with another sample if any image cannot be loaded.
        try:
            input_img = Image.open(os.path.join(self.input_folder, img_name)).convert("RGB")
            good_img = Image.open(os.path.join(self.good_folder, img_name)).convert("RGB")
            bad_img = Image.open(os.path.join(self.bad_folder, img_name)).convert("RGB")
        except Exception as e:
            print(f"Error loading {img_name}: {e}, skipping...")
            return self.__getitem__(random.randint(0, len(self)-1))

        # 2. Retrieve the caption.
        if self.captions:
            caption = self.captions.get(img_name, "")
        else:
            caption = "" # Use an empty caption when no prompt mapping is available.

        # 3. Apply the existing preprocessing logic.
        # Input (Conditioning) -> [0, 1]
        img_t_src = self.T(input_img)
        img_t_src = F.to_tensor(img_t_src)

        # Good/Bad (Targets) -> [-1, 1]
        # Normalize targets to [-1, 1], as in PairedDataset.
        img_t_good = self.T(good_img)
        img_t_good = F.to_tensor(img_t_good)
        img_t_good = F.normalize(img_t_good, mean=[0.5], std=[0.5])

        img_t_bad = self.T(bad_img)
        img_t_bad = F.to_tensor(img_t_bad)
        img_t_bad = F.normalize(img_t_bad, mean=[0.5], std=[0.5])
        ...
        return {
            "img_name": img_name,
            "pixel_values_src": img_t_src,   # [0, 1] for input
            "pixel_values_good": img_t_good, # [-1, 1] for target
            "pixel_values_bad": img_t_bad,   # [-1, 1] for target
            "input_ids": input_ids,
        }
