...
class Pix2Pix_Turbo(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_unet=8, lora_rank_vae=4, train_from_scratch=False, resume_from_scratch_ckpt=False,enable_skip_conv=True):
        ...
        if train_from_scratch:
            ...
            # ========== End skip-connection initialization ==========
            ...
            if pretrained_path is not None:
                ...
                if "model_state_dict" in sd and "rank_unet" not in sd:
                    raise ValueError(
                        "The file passed to pretrained_path is a plain model_state_dict snapshot "
                        "(for example full_model_*.pth), but this code path expects a LoRA checkpoint "
                        "with keys like rank_unet/rank_vae/unet_lora_target_modules. "
                        "If you want to continue from a self-supervised full checkpoint, use "
                        "--resume_from full_checkpoint_*.pt instead of --pretrained_path full_model_*.pth."
                    )
                ...
                if resume_from_scratch_ckpt:
                    ...
                    # ========== Fine-tuning without a supplied checkpoint ==========
                    # Initialize LoRA for the fine-tuning branch.
                    print("Fine-tune mode: initializing LoRA for UNet and VAE decoder only")
                    self.train_encoder_lora = True  # Enable encoder LoRA training in this implementation.
                    
                    # Initialize skip convolutions to small values.
                    torch.nn.init.constant_(vae.decoder.skip_conv_1.weight, 1e-5)
                    torch.nn.init.constant_(vae.decoder.skip_conv_2.weight, 1e-5)
                    torch.nn.init.constant_(vae.decoder.skip_conv_3.weight, 1e-5)
                    torch.nn.init.constant_(vae.decoder.skip_conv_4.weight, 1e-5)
                    
                    # Configure the VAE LoRA target modules.
                    target_modules_vae = self._collect_vae_target_modules(vae, include_encoder=False)
                    vae_lora_config = LoraConfig(
                        r=lora_rank_vae, init_lora_weights="gaussian",
                        target_modules=target_modules_vae
                    )
    ...
    def set_train(self):
        ...
        # print("\n===== DEBUG: VAE Encoder Parameters =====")
        # encoder_grad_count = 0
        # for n, p in self.vae.encoder.named_parameters():
        #     if p.requires_grad:
        #         print(f"Encoder param with grad: {n}, shape: {p.shape}")
        #         encoder_grad_count += 1
        # print(f"Total encoder params with requires_grad=True: {encoder_grad_count}")
        
        # print("\n===== DEBUG: VAE Decoder Parameters =====")
        # decoder_grad_count = 0
        # for n, p in self.vae.decoder.named_parameters():
        #     if p.requires_grad:
        #         print(f"Decoder param with grad: {n}, shape: {p.shape}")
        #         decoder_grad_count += 1
        # print(f"Total decoder params with requires_grad=True: {decoder_grad_count}")
    ...
    def forward(self, c_t, prompt=None, prompt_tokens=None, deterministic=True, r=1.0, noise_map=None):
        ...
        if deterministic:
            ...
            if noise_map is None:
                noise_map = torch.randn_like(encoded_control)
            encoded_noisy = encoded_control * r + noise_map * (1 - r)
            #encoded_noisy = self.sched.add_noise(encoded_control, noise_map, self.timesteps)
            model_pred = self.unet(encoded_noisy, self.timesteps, encoder_hidden_states=caption_enc,).sample
            x_denoised = self.sched.step(model_pred, self.timesteps, encoded_noisy, return_dict=True).prev_sample
            x_denoised = x_denoised.to(model_pred.dtype)
            self.vae.decoder.incoming_skip_acts = self.vae.encoder.current_down_blocks
            output_image = (self.vae.decode(x_denoised / self.vae.config.scaling_factor).sample).clamp(-1, 1)
        return output_image
    ...
    #     # ========== Initialize missing LoRA lora_B weights to zero ==========
    #     # A zero LoRA update preserves the base-layer output: base_layer(x) + lora_B @ lora_A @ x = base_layer(x).
    ...
    #             elif 'lora_A' in key:
    #                 # Keep the default lora_A initialization because lora_B is zero.
    #                 print(f"  Kept default lora_A: {key}")
    #             else:
    #                 print(f"  Warning: unexpected missing key: {key}")
    ...
    #     print("Loaded full scratch training checkpoint")
        
    #     # Load optimizer and other training state.
    ...
    #             train_loader.sampler.load_state_dict(ckpt["sampler_state"])
    #         except Exception as e:
    #             print(f"Warning: could not restore sampler state ({e})")
                
    #         # ========== Additional diagnostics ==========
    #     # Diagnostics before returning from load_full_checkpoint_for_scratch_training.
    #     print("\n" + "="*60)
    ...
    #     print(f"  Has lora_B: {hasattr(skip_1, 'lora_B')}")

    #     print("\n3️⃣ Skip Conv Weights:")
    #     if hasattr(skip_1, 'base_layer'):
    #         w = skip_1.base_layer.weight
    #         print(f"  base_layer.weight mean: {w.mean():.6f}")
    #         print(f"  base_layer.weight std: {w.std():.6f}")
    #         print(f"  base_layer.weight is_zero: {torch.allclose(w, torch.zeros_like(w), atol=1e-6)}")

    #     if hasattr(skip_1, 'lora_A'):
    #         for adapter_name in skip_1.lora_A.keys():
    #             lora_a = skip_1.lora_A[adapter_name].weight
    #             lora_b = skip_1.lora_B[adapter_name].weight
    #             print(f"  lora_A[{adapter_name}] mean: {lora_a.mean():.6f}")
    #             print(f"  lora_B[{adapter_name}] mean: {lora_b.mean():.6f}")
    #             print(f"  lora_B[{adapter_name}] is_zero: {torch.allclose(lora_b, torch.zeros_like(lora_b), atol=1e-6)}")
