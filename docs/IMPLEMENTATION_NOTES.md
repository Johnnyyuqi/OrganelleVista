...
The inspected xformers 0.0.22 metadata requires Torch 2.0.1; its native extension was built with Torch 2.0.1+cu118 / CUDA 11.8. The historical local environment instead runs Torch 2.0.1+cu117. The README's matching CUDA 11.8 builds have now passed fresh-install, GPU kernel, short training, and inference checks on RTX 4090 GPUs; see [validation details](VALIDATION.md#clean-environment-and-gpu-validation). The existing local environment was not modified. Both the [validated version snapshot](environment-validated.txt) and historical snapshot are records, not portable lockfiles.

`setuptools==75.6.0` is required for legacy `pkg_resources` imports in gdown and vision-aided-loss. A fresh Conda installation supplied setuptools 84.0.0, where that module is absent; the requirements pin resolves this compatibility issue.
